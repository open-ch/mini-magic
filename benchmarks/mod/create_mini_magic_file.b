...
for my $offset (1..$max_offset){
    ...
    $benchs->{$last_index} = sub { MimeType::create_mini_magic_file( \@sub, $MAGDIR, "$TMP_FILE/$last_index" ); };
}

if($max_offset*$step < $nbr_mimes){
    $benchs->{"all"} = sub { MimeType::create_mini_magic_file( \@shuffled_list, $MAGDIR, "$TMP_FILE/all" ); };
}
